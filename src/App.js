...
class App extends Component {
    ...
    if (title && description && image) {
      ...
      axios
        ...
        .then(response => {
          ...
          this.extractInfo(data, url, base_url, title, image, description);
        })
        .catch(error => {
  ...
  extractInfo(data, site_url, base_url, title, image, description){
  ...
  contentUpdated(website_object){
    // console.log('Content Updated in App.js', website_object);
    this.setState(website_object);
  }
  ...
  render() {
    return <main role="main" className="container">
        ...
        <div id="shareaction" className="effect5">
          <Preview url={this.state.url} title={this.state.title} image={this.state.image} description={this.state.description} errorImage={img_notfound} contentChangedEvent={this.contentUpdated.bind(this)} />
          <ShareButtons remoteSite={this.state} />
        </div>
        <UseIt />
        <About />
      </main>;
